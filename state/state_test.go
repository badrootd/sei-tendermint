...
import (
	...
	statefactory "github.com/badrootd/sei-tendermint/state/test/factory"
	"math"
	"math/big"
	mrand "math/rand"
	"os"
...
func setupTestCase(t *testing.T) (func(t *testing.T), dbm.DB, state2.State) {
	...
	state, err = state2.MakeGenesisStateFromFile(cfg.GenesisFile())
...
func TestMakeGenesisStateNilValidators(t *testing.T) {
	...
	state, err := state2.MakeGenesisState(&doc)
	require.NoError(t, err)
...
func TestValidatorSimpleSaveLoad(t *testing.T) {
	...
	assert.IsType(t, state2.ErrNoValSetForHeight{}, err, "expected err at height 0")
...
func TestStateProto(t *testing.T) {
	...
	tc := []struct {
		...
		{"empty state", &state2.State{}, true, false},
