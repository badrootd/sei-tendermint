...
func TestRollback(t *testing.T) {
	...
	// rollback the state
	rollbackHeight, rollbackHash, err := state2.Rollback(blockStore, stateStore, false, cfg.PrivValidator)
	require.NoError(t, err)
...
func TestRollbackNoState(t *testing.T) {
	cfg, _ := rpctest.CreateConfig(t, t.Name())
	stateStore := state2.NewStore(dbm.NewMemDB())
	blockStore := &mocks.BlockStore{}

	_, _, err := state2.Rollback(blockStore, stateStore, false, cfg.PrivValidator)
	require.Error(t, err)
...
func setupStateStore(t *testing.T, height int64) state2.Store {
	stateStore := state2.NewStore(dbm.NewMemDB())
...
func TestRollbackHard(t *testing.T) {
	...
	rollbackHeight, rollbackHash, err := state2.Rollback(
		blockStore,
		stateStore,
		true,
		cfg.PrivValidator,
	...
	rollbackHeight, rollbackHash, err = state2.Rollback(blockStore, stateStore, true, cfg.PrivValidator)
	require.NoError(t, err)
	require.Equal(t, rollbackHeight, currState.LastBlockHeight)
	require.Equal(t, rollbackHash, currState.AppHash)
}
