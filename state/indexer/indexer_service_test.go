...
import (
	...
	tmlog "github.com/badrootd/sei-tendermint/libs/log"
...
func TestIndexerServiceIndexesBlocks(t *testing.T) {
	...
	assert.False(t, indexer2.KVSinkEnabled([]indexer2.EventSink{}))
	assert.False(t, indexer2.IndexingEnabled([]indexer2.EventSink{}))

	// event sink setup
	pool := setupDB(t)

	store := dbm.NewMemDB()
	eventSinks := []indexer2.EventSink{kv.NewEventSink(store), pSink}
	assert.True(t, indexer2.KVSinkEnabled(eventSinks))
	assert.True(t, indexer2.IndexingEnabled(eventSinks))

	service := indexer2.NewService(indexer2.ServiceArgs{
		Logger:   logger,
		Sinks:    eventSinks,
		EventBus: eventBus,
	})
