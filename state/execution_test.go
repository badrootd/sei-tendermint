...
import (
	...
	abciclientmocks "github.com/badrootd/sei-tendermint/abci/client/mocks"
	abci "github.com/badrootd/sei-tendermint/abci/types"
	abcimocks "github.com/badrootd/sei-tendermint/abci/types/mocks"
	...
	mpmocks "github.com/badrootd/sei-tendermint/internal/mempool/mocks"
...
func TestApplyBlock(t *testing.T) {
	...
	blockExec := state2.NewBlockExecutor(stateStore, logger, proxyApp, mp, state2.EmptyEvidencePool{}, blockStore, eventBus, state2.NopMetrics())
...
func TestFinalizeBlockDecidedLastCommit(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			blockExec := state2.NewBlockExecutor(stateStore, log.NewNopLogger(), appClient, mp, evpool, blockStore, eventBus, state2.NopMetrics())
...
func TestFinalizeBlockByzantineValidators(t *testing.T) {
	...
	blockExec := state2.NewBlockExecutor(stateStore, log.NewNopLogger(), proxyApp, mp, evpool, blockStore, eventBus, state2.NopMetrics())
...
func TestValidateValidatorUpdates(t *testing.T) {
	...
	for _, tc := range testCases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			err := state2.ValidateValidatorUpdates(tc.abciUpdates, tc.validatorParams)
			if tc.shouldErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
