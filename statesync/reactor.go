...
func (r *Reactor) Sync(ctx context.Context) (state.State, error) {
	if r.eventBus != nil {
	...
	if r.syncer != nil {
		r.mtx.Unlock()
		return state.State{}, errors.New("a state sync is already in progress")
	}
	...
	stateObj, commit, err := r.syncer.SyncAny(ctx, r.cfg.DiscoveryTime, r.requestSnaphot)
	if err != nil {
		return state.State{}, err
	}

	if err := r.stateStore.Bootstrap(stateObj); err != nil {
		return state.State{}, fmt.Errorf("failed to bootstrap node with new state: %w", err)
	}

	if err := r.blockStore.SaveSeenCommit(stateObj.LastBlockHeight, commit); err != nil {
		return state.State{}, fmt.Errorf("failed to store last seen commit: %w", err)
	}

	if err := r.Backfill(ctx, stateObj); err != nil {
	...
	if r.eventBus != nil {
		if err := r.eventBus.PublishEventStateSyncStatus(types.EventDataStateSyncStatus{
			Complete: true,
			Height:   stateObj.LastBlockHeight,
		}); err != nil {
			return state.State{}, err
		}
	}
	...
	if r.postSyncHook != nil {
		if err := r.postSyncHook(ctx, stateObj); err != nil {
			return state.State{}, err
		}
	}
...
func (r *Reactor) Backfill(ctx context.Context, state state.State) error {
